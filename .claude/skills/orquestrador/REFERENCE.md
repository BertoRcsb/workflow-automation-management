...
- **`todos os boards`** — varre os três **em sequência, na ordem de prioridade do registro do `coletor` (incidentes SEMPRE 1º)**. Cada board é **totalmente isolado**: sua própria coleta → validação → versão-alvo → **sua própria página no Notion** (release/hotfix por board). **NUNCA** se misturam cards de boards diferentes (invariante §5.1 — a varredura reforça a regra, não a viola).
  ...
  - **Board sem mapeamento** → **pula e reporta** naquele bloco; os demais seguem normalmente. Definir filtro = descoberta via MCP + OK do usuário (ver `coletor`, "Registro de boards"). **Nunca inventar JQL.**
...
- **Passo 1** — `source: prerelease` → `target: teste_regressivo` (pré-prod): ajusta o YAML, **GATE-PROMO
  `--step passo1`**, `make dry-run` e **parseia**; **erro (exit 1) → documenta em `erros/` e para**; **limpo (exit 0) → emite a mensagem `Confira` e para**. **O `make run`** (abre os PRs) só **sob OK explícito do usuário**, depois da mensagem. Merge é do usuário.
- **Passo 2** — `source: teste_regressivo` → `target: master` (prod): **só sob comando/override explícito do usuário**. Por padrão o Optimus Prime edita o YAML (**troca `source` para `teste_regressivo` E `targets` para `master`**), roda **GATE-PROMO `--step passo2`** e `make dry-run`; o **`make run` de master só quando o usuário mandar explicitamente**.
- **Passo 3** — **`make run-triggers`** (dispara os triggers Cloud Build → deploy nos ambientes dos clientes; **NÃO recebe `PR_TITLE`**). O Optimus Prime pode **disparar** sob **comando explícito** do usuário; **quais** triggers vêm de **instrução do usuário + os `triggers:` do `repos.yaml`** (ainda **não** estão no Notion). A **aprovação do build no GCP é do usuário**, após OK do QA. Ex. (autocadastro-front): `neoenergia-front-autocadastro`, `vli-front-autocadastro`.
- **Pós-deploy (sync de volta)** — depois do deploy, sincroniza **master → develop, stage, prerelease**: edita o YAML (`source: master`, `targets: [develop, stage, prerelease]`), `make dry-run` → OK → **`make run PR_TITLE="Sync Master"`** (título **padrão** desse passo). Cria os PRs; **merges do usuário**.

**Governança:** master/prod, merges e aprovação de build = **sempre do usuário**; o Optimus Prime **pergunta antes** de master (Passo 2) e das triggers (Passo 3).
...
- **Deploy tool:** `sync-repos-from-master` — **repo separado** (não faz parte deste repo). Caminho via env **`SYNC_REPO_PATH`**; default: repo-irmão **`../sync-repos-from-master`**. Makefile. Comandos:
  ...
  - `make run-triggers` → dispara os **triggers Cloud Build** (deploy nos ambientes dos clientes). **NÃO recebe `PR_TITLE`.** Disparado pelo Optimus Prime **sob comando explícito do usuário**; a **aprovação do build no GCP** é do usuário (após OK do QA).
...
- **Refino:** o usuário revisa o `.md`; **só com o OK dele** a skill/comando que causou o erro é atualizada, e a melhoria é registrada. Erros viram base para deixar a esteira correta, limpa, segura e rápida.
