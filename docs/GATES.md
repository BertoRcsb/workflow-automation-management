...
| **Manifesto de fan-out** | `optimus_card_manifest.py` | CLI: `<busca-jql.json> <board> <data> [workers.json]` → manifesto JSON no stdout. Fatia as chaves da JQL em lotes (`batch_size` de `tools/workers.json`), fixa caminhos e `params` (o LLM não decide escopo nem concorrência); `fanout` só acima de `fanout_threshold`; ordem = ordem da JQL; chave duplicada = exit 1 (`erro=chave_duplicada`) | exit 1 → documenta e para |
| **Agregação de fan-out** | `optimus_card_aggregate.py` | CLI: `<manifesto.json> [override ...]` → contrato agregado (array) no stdout, ordem exata de `manifest.keys`, idêntico ao monolítico. Fail-closed: `lotes_ausentes=`/`faltantes=`/`duplicados=`/`intrusos=`/`formato_invalido=` no stderr + exit 1 (stdout vazio); overrides `*-refino-1-contrato.json` substituem por `card_id` | exit 1 → Optimus redespacha só os lotes faltantes (1x); persistindo, documenta e para |
...
| **GATE-VER-1/VER-2/VER-3** | `optimus_next_version.py` | Versão-alvo pelo maior semver numérico (nunca data); anti-colisão; completude do dump paginado (último `has_more` deve ser `false`) | exit 1 → decisão do usuário (VER-3: refazer a consulta paginando até `has_more: false`) |
